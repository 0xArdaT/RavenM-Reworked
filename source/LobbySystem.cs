...
namespace RavenM
{
    static class InstantActionMenuAccess
    {
        const BindingFlags Flags = BindingFlags.Instance | BindingFlags.NonPublic;

        public static MapEntryData GetSelectedMap(InstantActionConfigMenu menu)
        {
            return (MapEntryData)typeof(InstantActionConfigMenu).GetField("selectedMap", Flags).GetValue(menu);
        }

        public static int GetDropdownValue(InstantActionConfigMenu menu, string fieldName)
        {
            var dropdown = typeof(InstantActionConfigMenu).GetField(fieldName, Flags).GetValue(menu);
            return (int)dropdown.GetType().GetProperty("value").GetValue(dropdown, null);
        }

        public static void SetDropdownValue(InstantActionConfigMenu menu, string fieldName, int value)
        {
            var dropdown = typeof(InstantActionConfigMenu).GetField(fieldName, Flags).GetValue(menu);
            dropdown.GetType().GetProperty("value").SetValue(dropdown, value, null);
        }

        public static string GetInputText(InstantActionConfigMenu menu, string fieldName)
        {
            var input = typeof(InstantActionConfigMenu).GetField(fieldName, Flags).GetValue(menu);
            return (string)input.GetType().GetProperty("text").GetValue(input, null);
        }

        public static void SetInputText(InstantActionConfigMenu menu, string fieldName, string value)
        {
            var input = typeof(InstantActionConfigMenu).GetField(fieldName, Flags).GetValue(menu);
            input.GetType().GetProperty("text").SetValue(input, value, null);
        }

        public static UnityEngine.UI.Toggle GetToggle(InstantActionConfigMenu menu, string fieldName)
        {
            return (UnityEngine.UI.Toggle)typeof(InstantActionConfigMenu).GetField(fieldName, Flags).GetValue(menu);
        }
    }
    ...
    [HarmonyPatch(typeof(SkinPicker), "OnEnable")]
    ...
    public class LobbySystem : MonoBehaviour
    {
        ...
        private void StartAsClient()
        {
            ...
            InstantActionMenuAccess.SetInputText(menu, "botAmountEagleIF", "0");
            InstantActionMenuAccess.SetInputText(menu, "botAmountRavenIF", "0");
            menu.StartGame();
        }
        ...
        private void Update()
        {
            if (GameManager.instance == null || GameManager.IsIngame(out _) || GameManager.IsInLoadingScreen())
                return;
            ...
            if (InstantActionMenuAccess.GetDropdownValue(menu, "playerTeamDD") == 2)
            {
                InstantActionMenuAccess.SetDropdownValue(menu, "playerTeamDD", 0);
            }
            SetLobbyMemberDataDedup("team", InstantActionMenuAccess.GetDropdownValue(menu, "playerTeamDD") == 0 ? "<color=blue>E</color>" : "<color=red>R</color>");
            
            if (IsLobbyOwner)
            {
                SetLobbyDataDedup("gameMode", menu.selectedGameMode != null ? menu.selectedGameMode.name : "");
                SetLobbyDataDedup("nightMode", InstantActionMenuAccess.GetToggle(menu, "nightToggle").isOn.ToString());
                SetLobbyDataDedup("playerHasAllWeapons", GameManager.instance.gameModeParameters.playerHasAllWeapons.ToString());
                SetLobbyDataDedup("botAmountEagle", InstantActionMenuAccess.GetInputText(menu, "botAmountEagleIF"));
                SetLobbyDataDedup("botAmountRaven", InstantActionMenuAccess.GetInputText(menu, "botAmountRavenIF"));
                SetLobbyDataDedup("respawnTime", InstantActionMenuAccess.GetInputText(menu, "respawnTimeIF"));
                SetLobbyDataDedup("gameLength", InstantActionMenuAccess.GetDropdownValue(menu, "gameLengthDD").ToString());
                var selectedMap = InstantActionMenuAccess.GetSelectedMap(menu);
                SetLobbyDataDedup("loadedLevelEntry", selectedMap != null ? selectedMap.sceneName : "");
                if (menu.selectedGameMode == GameModeInfo.officialSpecOps)
                {
                    SetLobbyDataDedup("team", InstantActionMenuAccess.GetDropdownValue(menu, "playerTeamDD").ToString());
                }

                if (selectedMap != null && selectedMap.isCustomMap)
                {
                    selectedMap.GetOrLoadMetadata();
                    SetLobbyDataDedup("customMap", selectedMap.metaData.displayName);
                }
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    {
                        SetLobbyDataDedup(i + "vehicle_" + vehicleType, EncodeVehicleSlot(teamInfo, vehicleType));
                    }

                    foreach (var turretType in (TurretSpawner.TurretSpawnType[])Enum.GetValues(typeof(TurretSpawner.TurretSpawnType)))
                    {
                        SetLobbyDataDedup(i + "turret_" + turretType, EncodeTurretSlot(teamInfo, turretType));
                    }
                ...
                for (int i = 0; i < ModManager.instance.loadedMutators.Count; i++)
                {
                    ...
                    var configData = ModManager.GetMutatorConfiguration(mutator);
                    foreach (var item in mutator.configuration.GetAllFields())
                    {
                        string value = configData != null ? configData.SerializeField(item.id) : item.FormatValueAsString();
                        JSONNode node = new JSONString(value);
            ...
            {
                string gameModeName = SteamMatchmaking.GetLobbyData(ActualLobbyID, "gameMode");
                if (!string.IsNullOrEmpty(gameModeName) && (menu.selectedGameMode == null || menu.selectedGameMode.name != gameModeName))
                {
                    var gameModeInfo = GameManager.GetGameModeByName(gameModeName, null);
                    if (gameModeInfo != null)
                        menu.SetGameMode(gameModeInfo, false);
                }

                menu.ToggleNight(bool.Parse(SteamMatchmaking.GetLobbyData(ActualLobbyID, "nightMode")));
                GameManager.instance.gameModeParameters.nightMode = InstantActionMenuAccess.GetToggle(menu, "nightToggle").isOn;
                GameManager.instance.gameModeParameters.playerHasAllWeapons = bool.Parse(SteamMatchmaking.GetLobbyData(ActualLobbyID, "playerHasAllWeapons"));
                InstantActionMenuAccess.GetToggle(menu, "configFlagsToggle").isOn = false;
                InstantActionMenuAccess.SetInputText(menu, "botAmountEagleIF", SteamMatchmaking.GetLobbyData(ActualLobbyID, "botAmountEagle"));
                InstantActionMenuAccess.SetInputText(menu, "botAmountRavenIF", SteamMatchmaking.GetLobbyData(ActualLobbyID, "botAmountRaven"));
                InstantActionMenuAccess.SetInputText(menu, "respawnTimeIF", SteamMatchmaking.GetLobbyData(ActualLobbyID, "respawnTime"));
                InstantActionMenuAccess.SetDropdownValue(menu, "gameLengthDD", int.Parse(SteamMatchmaking.GetLobbyData(ActualLobbyID, "gameLength")));
                if (menu.selectedGameMode == GameModeInfo.officialSpecOps)
                {
                    InstantActionMenuAccess.SetDropdownValue(menu, "playerTeamDD", int.Parse(SteamMatchmaking.GetLobbyData(ActualLobbyID, "team")));
                }
                ...
                {
                    string sceneName = SteamMatchmaking.GetLobbyData(ActualLobbyID, "loadedLevelEntry");
                    string mapName = SteamMatchmaking.GetLobbyData(ActualLobbyID, "customMap");

                    if (InstantActionMenuAccess.GetSelectedMap(menu) == null || InstantActionMenuAccess.GetSelectedMap(menu).sceneName != sceneName)
                    {
                        var foundMap = FindMapEntry(sceneName, mapName);
                        if (foundMap != null)
                        {
                            menu.SelectMap(foundMap);
                            doubleCheck = true;
                        }
                    }
                }
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    bool changedVehicles = ApplyVehicleSlotFromLobby(teamInfo, i);
                    bool changedTurrets = ApplyTurretSlotFromLobby(teamInfo, i);

                    if (changedVehicles || changedTurrets)
                        GamePreview.UpdatePreview();

                    string skinName = SteamMatchmaking.GetLobbyData(ActualLobbyID, i + "skin");
                    if (!string.IsNullOrEmpty(skinName))
                    {
                        var skin = ActorSkin.FromName(skinName);
                        if (skin != null)
                            teamInfo.skin = skin;
                    }
                }
                ...
                foreach (var mutatorStr in enabledMutators)
                {
                    ...
                    for (int mutatorIndex = 0; mutatorIndex < ModManager.instance.loadedMutators.Count; mutatorIndex++)
                    {
                        ...
                        if (id == mutatorIndex)
                        {
                            ...
                            var configData = ModManager.GetMutatorConfiguration(mutator);
                            var fields = mutator.configuration.GetAllFields().ToList();

                            for (int i = 0; i < fields.Count && i < config.Length; i++)
                            {
                                var item = fields[i];
                                if (configData != null && !string.IsNullOrEmpty(config[i]))
                                    configData.Deserialize(item.id, config[i]);
                            }
                        }
                    }
                }
        ...
        private static MapEntryData FindMapEntry(string sceneName, string displayName)
        {
            if (!string.IsNullOrEmpty(sceneName))
            {
                foreach (var map in ModManager.AllAvailableMaps())
                {
            ...
            {
                foreach (var map in ModManager.AllCustomMaps())
                {
                    map.GetOrLoadMetadata();
                    if (map.hasLoadedMetaData && map.metaData.displayName == displayName)
                        return map;
                }
            }
        ...
        private string EncodeVehicleSlot(TeamInfo teamInfo, VehicleSpawner.VehicleSpawnType type)
        {
            if (!teamInfo.vehicleSlot.TryGetValue(type, out var slot))
                return "NULL";

            var parts = new List<string>();
            foreach (var vehicle in slot.All())
            {
                if (vehicle == null || vehicle.prefab == null)
                    continue;

                bool isDefault = true;
                int idx = Array.IndexOf(ActorManager.instance.defaultVehiclePrefabs, vehicle.prefab);
                if (idx == -1)
                {
                    isDefault = false;
                    idx = sortedModdedVehicles.IndexOf(vehicle.prefab);
                }
                ...
                slot.IsAvailable(vehicle, out tier);
                parts.Add(isDefault + "," + idx + "," + (int)tier);
            }

            return parts.Count == 0 ? "NULL" : string.Join(";", parts.ToArray());
        }

        private string EncodeTurretSlot(TeamInfo teamInfo, TurretSpawner.TurretSpawnType type)
        {
            ...
            var parts = new List<string>();
            foreach (var turret in slot.All())
            {
                if (turret == null || turret.prefab == null)
                    continue;

                bool isDefault = true;
                int idx = Array.IndexOf(ActorManager.instance.defaultTurretPrefabs, turret.prefab);
                if (idx == -1)
                {
                    isDefault = false;
                    idx = moddedTurrets.IndexOf(turret.prefab);
                }
                ...
                RarityTier tier = RarityTier.Common;
                slot.IsAvailable(turret, out tier);
                parts.Add(isDefault + "," + idx + "," + (int)tier);
            }
        ...
        private bool ApplyVehicleSlotFromLobby(TeamInfo teamInfo, int teamIndex)
        {
            bool changed = false;
            foreach (var vehicleType in (VehicleSpawner.VehicleSpawnType[])Enum.GetValues(typeof(VehicleSpawner.VehicleSpawnType)))
            {
                string encoded = EncodeVehicleSlot(teamInfo, vehicleType);
                string target = SteamMatchmaking.GetLobbyData(ActualLobbyID, teamIndex + "vehicle_" + vehicleType);
                if (encoded == target)
                    continue;

                changed = true;
                if (teamInfo.vehicleSlot.TryGetValue(vehicleType, out var slot))
                    slot.ClearEntries();

                if (string.IsNullOrEmpty(target) || target == "NULL")
                    continue;

                foreach (string entry in target.Split(';'))
                {
                    var prefab = ResolvePrefabFromLobbyEntry(entry, true);
                    if (prefab == null)
                        continue;
                    var info = VehicleInfo.GetFromPrefab(prefab);
                    if (info != null)
                        teamInfo.AddVehicle(vehicleType, ParseTierFromLobbyEntry(entry), info);
                }
            }
            return changed;
        }

        private bool ApplyTurretSlotFromLobby(TeamInfo teamInfo, int teamIndex)
        {
            bool changed = false;
            foreach (var turretType in (TurretSpawner.TurretSpawnType[])Enum.GetValues(typeof(TurretSpawner.TurretSpawnType)))
            {
                string encoded = EncodeTurretSlot(teamInfo, turretType);
                string target = SteamMatchmaking.GetLobbyData(ActualLobbyID, teamIndex + "turret_" + turretType);
                if (encoded == target)
                    continue;

                changed = true;
                if (teamInfo.turretSlot.TryGetValue(turretType, out var slot))
                    slot.ClearEntries();
                ...
                foreach (string entry in target.Split(';'))
                {
                    var prefab = ResolvePrefabFromLobbyEntry(entry, false);
                    if (prefab == null)
                        continue;
                    var info = VehicleInfo.GetFromPrefab(prefab);
                    if (info != null)
                        teamInfo.AddTurret(turretType, ParseTierFromLobbyEntry(entry), info);
                }
            }
            return changed;
        }

        private GameObject ResolvePrefabFromLobbyEntry(string entry, bool vehicle)
        {
            if (string.IsNullOrEmpty(entry) || entry == "NULL")
                return null;
            ...
            if (isDefault)
                return vehicle ? ActorManager.instance.defaultVehiclePrefabs[idx] : ActorManager.instance.defaultTurretPrefabs[idx];

            if (vehicle)
                return idx >= 0 && idx < sortedModdedVehicles.Count ? sortedModdedVehicles[idx] : null;

            var moddedTurrets = ModManager.AllTurretPrefabs().ToList();
            return idx >= 0 && idx < moddedTurrets.Count ? moddedTurrets[idx] : null;
        }

        private static RarityTier ParseTierFromLobbyEntry(string entry)
        {
            string[] args = entry.Split(',');
            if (args.Length >= 3 && int.TryParse(args[2], out int tier) && Enum.IsDefined(typeof(RarityTier), tier))
        ...
        private void OnGUI()
        {
            if (GameManager.instance == null || (GameManager.IsIngame(out _) && LoadoutUi.instance != null && LoadoutUi.HasBeenClosed()))
                return;
